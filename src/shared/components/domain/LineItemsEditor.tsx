...
interface LineItemsEditorProps {
  items: LineItem[];
  itemForm: LineItemFormData;
  onItemFormChange: (form: LineItemFormData) => void;
  onAddItem: () => void;
          ...
          <div className="divide-y divide-gray-200 dark:divide-gray-600">
            {items.map((item) => (
              <div key={item.id} className="p-4 flex items-center gap-4 hover:bg-gray-50 dark:hover:bg-gray-600 transition-colors">
                ...
                <Button size="sm" variant="outline" type="button" onClick={() => onRemoveItem(item.id)} className="px-4 py-2 text-base">
                  Quitar
                </Button>
              </div>
            ))}
